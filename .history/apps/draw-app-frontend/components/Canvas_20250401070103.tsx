...
type t = "circle" | "rect" | "pencil"

export function Canvas({ roomId, socket }: { roomId: string; socket: WebSocket }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const { width, height } = useDimension(canvasRef);
  const [selectedTool, setSelectedTool] = useState<t>("circle")

  useEffect(() => {
    //@ts-ignore
    window.selectedTool = selectedTool;
  },[])//We'll make a class out of game engine
...
function Topbar({selectedTool,setSelectedTool} : {selectedTool : t, setSelectedTool: (s : t) => void }){
    return <div style={{
        position:"fixed",
        top:10,
        left: "50%",
